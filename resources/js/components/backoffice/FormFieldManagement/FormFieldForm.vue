<template>
    <el-dialog
        ...
        @close="closeForm"
        :title="hasModel ? __('Update Form Field') : __('Add New Form Field')"
        width="40%"
        top="4vh"
        append-to-body>
        <el-form
            ...
            <div class="row">
                <div class="col-md-6">
                    <el-form-item
                        :label="__('Form Group')"
                        prop="form_field_group_id">
                        <el-select
                            class="full-width round-input"
                            :placeholder="__('Select Group')"
                            filterable
                            clearable
                            v-model="formFieldForm.form_field_group_id">
                            <el-option
                                v-for="group in formGroups"
                                :label="__dt(group.translations, 'name', $sf.ucwords(group.name)) || group.name"
                                :value="group.id"
                                :key="group.id">
                            </el-option>
                        </el-select>
            ...
            <transition name="fade">
                <el-button
                    v-if="(selectedType.type === 'option' && selectedSource && !selectedSource.value) || (selectedType.type === 'matrix' && !formFieldForm.source_table)"
                    round
                    size="mini"
                    class="btn btn-black"
            ...
            <template v-if = "allowToLink">
                <el-form-item
                    ...
                    <el-switch v-model="formFieldForm.is_link_to_table" :disabled = "formFieldForm.field_type === 'Matrix'"
                    @change="linkToTableChange"></el-switch>
                </el-form-item>
                ...
                <template v-if="formFieldForm.is_link_to_table && formFieldForm.field_type !== 'Matrix'">
                    <el-form-item
                        ...
                        prop="table_column">
                        <el-cascader
                            v-model="formFieldForm.table_column"
...
export default {
    ...
    data() {
        return {
            ...
            rules: {
                //form_field_group_id: {required: true, message: this.__("This field is required"), trigger: "change"},
    ...
    created() {
        ...
        if (this.model && this.model.id) {
            this.formFieldForm = {
                id: this.model.id,
                form_field_group_id: this.model.form_field_group_id,
    ...
    methods: {
        ...
        /*getFields(value) {
            /*if (this.selectedOption) {
                this.$API.FormField.getFields('Country')
                .then(res => {
                    this.formFieldForm.field_options = cloneDeep(res.data)
                })
                .catch(err => {
                    console.log(err)
                })
        ...
        create() {
            ...
            if(postData.field_type === 'Matrix'){
                if(postData.source_table){
                    postData.table_name = postData.source_table
                }
                else{
            ...
            this.$API.FormField.create(postData)
            .then(res => {
                if (res.data.data) {
                    this.$notify({
                    ...
                    this.$EventDispatcher.fire("FORM_FIELD_MODULE_ITEM_CREATED", res.data.data);
        ...
        update() {
            ...
            this.$API.FormField.update(postData, postData.id)
            .then(res => {
                if (res.data.data) {
                    this.$notify({
                        title: this.__("Successful"),
                    ...
                    this.$EventDispatcher.fire("FORM_FIELD_MODULE_ITEM_UPDATED", res.data.data);
        ...
        linkToTableChange() {
            if(this.formFieldForm.is_link_to_table == false) {
